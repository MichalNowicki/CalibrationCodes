...
%   linewidth.
%
%   W [default = 1/72 of the PlotBox (PositionRectangle) diagonal for
%   linear (log) plots] is a vector of arrowhead widths.  For linear
%   plots with equal axes, the units of W are the same as those of the
%   coordinate data (P1,P2).  For linear plots with unequal axes, the
%   units of W are scaled to fit as if the axes were equal.  For log
%   plots, the units of W are 1/72 of the PositionRectangle diagonal.
...
%   with diameter IP; for default diameter W, use IP = 0.
%
%   ALPHA (default = 1) is a vector of FaceAlpha values ranging between
%   0 (clear) and 1 (opaque).  FaceAlpha is a surface (arrowhead and
%   initial point marker) property and does not affect lines.  BETA
%   (default = 0.4) is a scalar that controls the relative brightness
%   of light and dark color shades, ranging between 0 (no contrast) and
%   1 (maximum contrast).
%
...
%   ARROW3 may alter the appearance of arrowheads and initial point
...
if strcmp(get(ax,'nextplot'),'add') & strcmp(get(fig,'nextplot'),'add')
   ...
   xys=xs+ys; restore=1;
   if xys & c1==3 & any([p1(:,c1);p2(:,c1)]~=0)
      error('3D log plot not supported')
   end
else, restore=0; cla; view(c1); xys=0;
   set(ax,'nextplot','add'); set(fig,'nextplot','add');
end
...
for i=1:n                            % translate, rotate, and scale
   ...
   LocalSetSurface(xys,xs,ys,dx,dy,xr,yr,x,y,z,a(i),c(i,:),H2(i));
end, delete(G);
...
if any(ip>0)
   ...
   for i=1:n                         % translate
      if ip(i)>0
         ...
         LocalSetSurface(xys,xs,ys,dx,dy,xr,yr,x,y,z,a(i),c(i,:),H3(i));
      end
   end, delete(G);
else, H3=[]; end
...
%==========================================================================
% Finish
if xys, set(ax,'DataAspectRatioMode','auto');
   xr=Xr; yr=Yr; zr=Zr;
...
%==========================================================================
% Update
function LocalUpdate(fig,ax,ud)
p1=ud(:,1:3); p2=ud(:,4:6); c=ud(:,7:9);
w=ud(:,10); h=ud(:,11); ip=ud(:,12); a=ud(:,13);
H=get(ax,'children'); tag=get(H,'tag'); type=get(H,'type');
delete(H(strcmp(tag,'arrow3') & strcmp(type,'surface')));
set(ax,'ColorOrder',c,'nextplot','add'); set(fig,'nextplot','add');
arrow3(p1,p2,'o0',w,h,ip,a);

%==========================================================================
% SetSurface
function LocalSetSurface(xys,xs,ys,dx,dy,xr,yr,x,y,z,a,c,H)
if xys
